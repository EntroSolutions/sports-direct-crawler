...
@include('crud::fields.inc.wrapper_start')
    ...
    <div class="row">
        <div class="col-sm-6" data-handle="previewArea" style="margin-bottom: 20px;">
            <img data-handle="mainImage" src="{{ $field['colorImage']->image }}">
        </div>
    ...
    <div class="btn-group">
        <div class="btn btn-light btn-sm btn-file">
            ...
            <input type="hidden" data-handle="hiddenImage" name="{{ $field['name'] }}" data-value-prefix="{{ $field['prefix'] }}" value="{{ $field['colorImage']->image }}">
        </div>
        @if(isset($field['crop']) && $field['crop'])
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            function bpFieldInitCropperImageElement(element) {
                    ...
                    let previx = '';
                    if($hiddenImage.val().indexOf('http') == -1){
                        previx = '/';
                    }
                    $mainImage.attr('src', previx + $hiddenImage.val());
